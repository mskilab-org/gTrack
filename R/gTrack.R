...
setMethod('initialize', 'gTrack', function(.Object,
                                           ...
                                           ...)
          {
              ...
              .Object@edges[edges.mat] = lapply(which(edges.mat), function(x)
                     {
                       ...
                       if (nrow(y) != length(.Object@data[[x]]) & ncol(y) != length(.Object@data[[x]]))
                         warning('dimensions of matrix do not match length of track data item.  Adjacency matrix of edges should be square and have as many rows as there are input ranges in the correspondnig gTrack item')
                       tmp.edges = which(y>0, arr.ind = T)
            ...
            if (is.null(name))
              ...
              .Object@formatting = data.frame(track.name = t.name, height = height, ygap = ygap, stack.gap = stack.gap, lift = lift, split = split, angle = angle, format = format, col = col, lwd.border = lwd.border, ypad = ypad,  ywid = ywid, border = border, hadj.label = hadj.label, gr.colorfield = gr.colorfield, smooth = smooth, round = round, vadj.label = vadj.label, legend = legend, y.field = y.field, circles = circles, bars = bars, y0.bar = y0.bar, lines = lines, source.file.chrsub = source.file.chrsub, yaxis.cex = yaxis.cex, cex.tick = cex.tick, max.ranges = max.ranges, yaxis = yaxis, yaxis.pretty = yaxis.pretty, y.quantile = y.quantile, triangle = triangle, is.null = is.null,
            ...
            if (is.null(seqinfo) & !is.null(data))
              {
                slen = c()
                    ...
                    if (is(x, 'GRanges') | is(x, 'GRangesList'))
                      {
                        if (is.null(slen))
                          ...
                          slen[seqlevels(x)] = pmax(slen[seqlevels(x)], seqlengths(x), na.rm = T)
                      }
...
#                              stop('External file must be a valid and existing .bed file')
                        ...
                        else if (grepl('\\.rds', x, ignore.case = T)) ## assume this is GRanges or GRangesList
                          {
                      ...
                      if (is.null(slen))
                        ...
                        slen[names(x)] = pmax(slen[x], sapply(x, length), na.rm = T)
                  }
...
setValidity('gTrack', function(object)
          {
            ...
            if (length(object@data)>0)
              if (!all(sapply(object@data, function(x) class(x) %in% ALLOWED.DATA.CLASSES)))
                ...
                {
                  for (i in 1:length(object@formatting$format))
                ...
                for (j in 1:length(.Object))
                  {
                    ...
                    if (formatting(.Object[j])$format != 'ranges')
                      stop("violated assumption. need to fix")
                        ...
                        plot.track.formats[names(other.formats)] = other.formats;

                        plot.track.formats = plot.track.formats[setdiff(names(plot.track.formats), setdiff(na.fields, c('col')))]
...
#                              range.y = range(setdiff(values(dat(.Object)[[j]])[, this.y.field], c(Inf, -Inf)), na.rm = T);
                            ...
                            if (!is.null(formatting(.Object)$y.cap)) ## cap values from top and bottom
                              ...
                              this.y = affine.map(values(tmp.dat)[, this.y.field], ylim = unlist(this.ylim.subplot[j, ]), xlim = range(this.y.ticks), cap = T)
#                            this.y = affine.map(values(dat(.Object)[[j]])[, this.y.field], ylim = unlist(this.ylim.subplot[j, ]), xlim = range(this.y.ticks))
                        ...
                        if (!is.null(plot.track.formats$bars))
                          if (plot.track.formats$bars)
                            if (is.null(plot.track.formats$y0.bar))
                              plot.track.formats$y0.bar = this.ylim.subplot[j, 1]
...
draw.grl = function(grl,
  ...
  views = NULL, ## deprecated alias for windows
  windows = views,  # windows specifies windows, can have optional meta data features $col and $border
  win.gap = NULL,
  stack.gap = 100,
  ...
  xaxis.suffix = "",  ### DEFAULT GOES HERE
  xaxis.unit = 1, ##1e6,
  xaxis.round = 3,
  xaxis.interval = NULL,
  xaxis.pos = NULL,
  ...
  y0.bar = NULL,
  lines = F,
  angle = 20, # angle of barbs to indicate directionality of ranges
  pintersect=FALSE, # send to gr.flatmap which sends to gr.findoverlaps
  verbose=FALSE,
  ...
  mdata=NULL, ## matrix of data for plotting triangle
  cmap.min=NULL, ## minimum range of the colormap. will efault to min(mdata)
  cmap.max=NULL, ## maximum range of the colormap
  m.sep.lwd=1,    ## border weight of triangle plot
  m.bg.col = 'gray80',
...
#          draw.backbone = is.null(y)
        ...
        if (is.na(labels.suppress.grl))
          labels.suppress.grl = F

        if (is.na(labels.suppress.gr))
          labels.suppress.gr = F
