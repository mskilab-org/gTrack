...
You can plot karyograms in gTrack. If no file is passed to the `karyogram` method as an argument, it will use the karyogram datafiles (hg18 or hg19, depending on if the `r hg19` parameter is set to `r FALSE`) included with the package. You can specify if you colored giemsa bands by setting the `r bands` parameter to `r TRUE` or `r FALSE` (enabled by default). You can also return the chromosome arms with different colors, and with centromeres and telemeres marked, by setting `r arms = TRUE` (also enabled by default) 
