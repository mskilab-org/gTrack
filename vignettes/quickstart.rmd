...
```{r include = TRUE, message = FALSE, warning = FALSE, out.height = 10} 
...
A `gTrack` can also be created from a `GRangesList`. This is desirable when plotting ranges that are grouped together in some way, such as alignments deriving from a single read pair in the case of short read whole genome sequencing.

# Matrix

A `gTrack` can be created from a `GRanges` and corresponding `Matrix` in order to plot associations between two genomic ranges. One example of this would be a heatmap, where the color of each cell is proportional to some value defined by its corresponding genomic regions, such as number of shared read pairs.
