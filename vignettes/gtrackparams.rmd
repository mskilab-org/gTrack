...
The label of the Y-axis on the left side of a `gTrack` can be toggled on or off using the logical parameter `yaxis`, which has default value `TRUE` if `y.field` is specified and `FALSE` if not.
...
tiles.gt = gTrack(tiles)

## stack intervals within 1 Kbp of each other
tiles.stack.gt = gTrack(tiles, stack.gap = 1000)
...
## only plot 50 points
coverage.downsample.gt = gTrack(coverage.gr, y.field = "cn", circles = TRUE, lwd.border = 0.2, max.ranges = 50)
